...
```bash
daml build
daml codegen js .daml/dist/js-daml-erc-20-app-0.1.0.dar -o ui/daml.js
cd ui
npm install
npm run-script build
zip -r ../js-daml-erc-20-app-ui.zip build
```

Next you need to create a ledger on [Daml Hub], upload the files
`.daml/dist/js-daml-erc-20-app-0.1.0.dar` (created by the `daml build` command)
and `js-daml-erc-20-app-ui.zip` (created by the `zip` command based on the result
